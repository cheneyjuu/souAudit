...
export class SysdataZxSynchdatatableComponent implements OnInit {
  ...
  editInfo(record: any[]) {
    this.modal.create(SysdataZxSynchdatatableEditComponent, { record }, { size: 'lg' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }

  viewInfo(record: any[]) {
    this.modal.create(SysdataZxSynchdatatableViewComponent, { record }, { size: 'lg' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }
